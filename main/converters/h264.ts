...
import {
  areDimensionsEven, conditionalArgs, ConvertOptions, makeEven,
} from './utils';
...
const convertToGif = PCancelable.fn(async (options: ConvertOptions, onCancel: PCancelable.OnCancelFunction) => {
  ...
  const paletteProcess = convert(palettePath, {shouldTrack: false}, conditionalArgs('-i', options.inputPath, '-vf', `fps=${options.fps}${options.shouldCrop ? `,scale=${options.width}:${options.height}:flags=lanczos` : ''},palettegen`, {
    args: [
      '-ss',
...
const convertToMp4 = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  startTime: options.startTime,
  endTime: options.endTime,
}, conditionalArgs('-i', options.inputPath, '-r', options.fps.toString(), {
  args: ['-an'],
...
const convertToWebm = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  '-threads', Math.max(os.cpus().length - 1, 1).toString(), '-deadline', 'good', // `best` is twice as slow and only slighty better
  '-b:v', '1M', // Bitrate (same as the MP4)
  '-codec:v', 'vp9', '-codec:a', 'vorbis', '-ac', '2', // https://stackoverflow.com/questions/19004762/ffmpeg-covert-from-mp4-to-webm-only-working-on-some-files
...
const convertToAv1 = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  '-i', options.inputPath, '-r', options.fps.toString(), '-c:v', 'libaom-av1', '-c:a', 'libopus', '-crf', '34', '-b:v', '0', '-strict', 'experimental',
...
const convertToHevc = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  '-i', options.inputPath, '-r', options.fps.toString(), '-c:v', 'libx265', '-c:a', 'libopus', '-preset', 'medium', '-tag:v', 'hvc1', // Metadata for macOS
  {
    args: ['-an'],
    if: options.shouldMute,
  }, {
...
const convertToApng = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  '-i', options.inputPath, '-vf', `fps=${options.fps}${options.shouldCrop ? `,scale=${options.width}:${options.height}:flags=lanczos` : ''}`,
...
export const crop = (options: ConvertOptions) => convert(options.outputPath, {
  ...
  startTime: options.startTime,
  endTime: options.endTime,
}, conditionalArgs('-i', options.inputPath, '-s', `${makeEven(options.width)}x${makeEven(options.height)}`, '-ss', options.startTime.toString(), '-to', options.endTime.toString(), options.outputPath));
