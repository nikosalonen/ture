...
Run `pnpm dev` in one terminal tab to start watch mode, and in another tab, run `pnpm start` to launch Kap.
...
- GitHub Actions will now build the app and add the binaries to the release
- When GitHub Actions has attached the binaries to the release, click `Edit` on the release, and then click `Publish release`
...
- Wait for GitHub Actions to add the binaries to a new GitHub Releases draft
